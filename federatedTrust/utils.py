...
def read_system_metrics_log(outdir, file):
    ...
    with open(os.path.join(outdir, file), 'r') as f:
        ...
        for line in lines:
            ...
            elif dict_dat.get('id') == 'sys_std':
                continue
            else:
                total_upload += dict_dat.get('total_upload_bytes')
                total_download += dict_dat.get('total_download_bytes')
    return [result, total_upload, total_download]
...
def convert_string_to_number(number:str):
    if not number:
        return None
    split = number.split(".")
    before_comma = split[0]
    after_comma = split[1]
    num = float(before_comma + "." + after_comma[0:-1])
    magnitude = after_comma[-1]
    if magnitude == 'M':
        return num * 10**6
    elif magnitude == 'B':
        return num * 10**9
